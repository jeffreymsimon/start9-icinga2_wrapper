...
const filledAddress = (host, addressInfo) => {
    ...
    function filledAddressFromHostnames(hostnames) {
        return {
            ...
            get nonLocal() {
                return filledAddressFromHostnames(filterRec(hostnames, nonLocalFilter, false));
            },
            get public() {
                return filledAddressFromHostnames(filterRec(hostnames, publicFilter, false));
            },
            get onion() {
                return filledAddressFromHostnames(filterRec(hostnames, onionFilter, false));
            },
        };
    }
...
class GetServiceInterface {
    constructor(effects, opts, map, eq) {
    ...
    async *watchGen(abort) {
        ...
        while (this.effects.isInContext && !abort?.aborted) {
            ...
            const next = this.map(await makeInterfaceFilled({
                effects: this.effects,
                id,
                packageId,
                callback,
...
function getOwnServiceInterface(effects, id, map, eq) {
    return new GetServiceInterface(effects, { id }, map ?? ((a) => a), eq ?? ((a, b) => (0, deepEqual_1.deepEqual)(a, b)));
}
function getServiceInterface(effects, opts, map, eq) {
    return new GetServiceInterface(effects, opts, map ?? ((a) => a), eq ?? ((a, b) => (0, deepEqual_1.deepEqual)(a, b)));
}
